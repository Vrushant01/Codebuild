...
export default function ReceptionistProfilePage() {
  ...
  // Manually read mock permissions for display
  const permissions = {
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card border rounded-3xl p-6 sm:p-8 shadow-sm">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              <div className="w-24 h-24 rounded-full bg-primary/10 flex items-center justify-center text-primary shrink-0 relative">
                {user?.avatar ? (
              ...
              <div className="flex-1 text-center sm:text-left space-y-4 w-full">
                <div>
                  <h2 className="text-2xl font-bold">{user?.name || "Receptionist User"}</h2>
                ...
                <div className="space-y-3 pt-4 border-t">
                  <div className="flex items-center justify-center sm:justify-start gap-3 text-sm">
                    ...
                    <span>CityCare Clinic</span>
                  </div>
                  <div className="flex items-center justify-center sm:justify-start gap-3 text-sm">
                    <Mail className="w-4 h-4 text-muted-foreground" />
                    <span>{user?.email || "reception@citycareclinic.demo"}</span>
                  </div>
                  <div className="flex items-center justify-center sm:justify-start gap-3 text-sm">
                    <Phone className="w-4 h-4 text-muted-foreground" />
                    <span>{user?.mobile || "+91 XXXXX XXXXX"}</span>
                  </div>
                </div>
              </div>
            </div>
