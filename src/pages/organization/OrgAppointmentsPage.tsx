      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredApts.map(apt => (
            <div key={apt.id} className="bg-card border rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col sm:flex-row gap-4 justify-between sm:items-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 w-full sm:w-auto">
                ...
                <div className="flex gap-4 min-w-[200px]">
                  <div className="w-12 h-12 rounded-xl bg-primary/10 text-primary flex items-center justify-center shrink-0">
                    <span className="font-bold text-sm text-center leading-tight">
                      {apt.date.split("-")[2]}<br/><span className="text-[10px] uppercase opacity-70">Day</span>
                    </span>
                  </div>
                  <div>
                    <h4 className="font-bold">{apt.patientName}</h4>
                    <p className="text-xs text-muted-foreground mt-0.5">ID: {apt.patientId}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{apt.timeStr}</p>
                </div>
