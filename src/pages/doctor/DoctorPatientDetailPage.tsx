...
export default function DoctorPatientDetailPage() {
  ...
  const loadData = async () => {
    ...
    const [pList, cCases, pCases, aList, mList] = await Promise.all([
      doctorService.getPatients(), // normally getPatientById
      ...
      scheduleService.getMedicines() // normally by patient
    ])
    setPatient(pList.find(p => p.patientId === patientId) || pList[0])
