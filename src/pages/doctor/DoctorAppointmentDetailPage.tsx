...
import {
  ...
  Edit3, Pill, User, XCircle, Check, AlertTriangle
} from "lucide-react"
...
export default function DoctorAppointmentDetailPage() {
  ...
  const refresh = async () => {
    ...
    if (a) setApt(a)
    const m = await scheduleService.getMedicines()
    setMedicines(m)
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="w-full lg:w-[380px] shrink-0 space-y-4">
          ...
          <div className="bg-card border rounded-3xl p-5 shadow-sm space-y-3">
            ...
            {isConfirmed && apt.consultationType === "Online" && (
              <Button onClick={() => navigate(`/app/telemedicine/${apt.id}`)} className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-2xl py-5">
      ...
      <AddMedicationModal
        ...
        onSave={async (data) => { 
           await doctorService.addMedication(apt.id, data) 
           await refresh()
        }}
      />
    </div>
