  ...
  return (
    <div className="bg-card border rounded-xl p-4 sm:p-5 flex flex-col gap-4 hover:border-primary/50 hover:shadow-sm transition-all duration-200">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className={cn(
          "px-2.5 py-1 rounded-md text-[10px] font-semibold border flex items-center gap-1.5",
          isFull 
            ? "bg-muted text-muted-foreground border-border" 
            : "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800"
        )}>
          <div className={cn("w-1.5 h-1.5 rounded-full", isFull ? "bg-muted-foreground" : "bg-emerald-500 animate-pulse")} />
          {isFull ? "Fully booked" : `Available ${doctor.availability.nextAvailable}`}
        </div>

        {hasOnline && (
          <div className="px-2.5 py-1 rounded-md text-[10px] font-semibold border bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800 flex items-center gap-1">
