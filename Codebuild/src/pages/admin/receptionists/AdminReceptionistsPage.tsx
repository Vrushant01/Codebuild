      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredReceptionists.map(r => {
            const orgName = typeof r.organizationId === 'object' ? r.organizationId?.name : (r.organizationName || r.organizationId || "Hospital")
            return (
              <div key={r.id || r._id} className="bg-card border rounded-2xl p-5 shadow-sm">
                <h3 className="font-bold mb-1">{r.name}</h3>
                <p className="text-sm text-muted-foreground mb-3">{r.email}</p>
                <div className="flex items-center gap-2 text-xs">
                  <span className="bg-muted px-2 py-1 rounded-md">{orgName}</span>
                  <span className={`px-2 py-1 rounded-md font-bold uppercase tracking-wider ${
                    (r.status || 'Active') === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                  }`}>
                    {r.status || 'Active'}
                  </span>
                </div>
