...
import { ManageReceptionistModal } from "../../components/organization/ManageReceptionistModal"
...
export default function OrgReceptionistsPage() {
  ...
  const [selectedStaff, setSelectedStaff] = useState<Receptionist | null>(null)
  ...
  const handleOpenManage = (person: Receptionist) => {
    setSelectedStaff(person)
    setIsManageModalOpen(true)
  }

  const handleSaveReceptionist = async (id: string, data: any) => {
    await organizationService.updateReceptionist(id, data)
    await loadData()
  }

  const handleDeleteReceptionist = async (id: string) => {
    await organizationService.removeReceptionist(id)
    await loadData()
  }
          ...
          {filtered.map(person => (
            <div key={person.id} className="bg-card border rounded-3xl p-5 shadow-sm flex flex-col md:flex-row gap-6 justify-between md:items-center">
              ...
              <div className="flex items-center gap-3">
                <button 
                  onClick={() => handleOpenManage(person)}
