                ...
                <div key={apt.id} className="bg-card border rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row gap-4 justify-between sm:items-center">
                  <div className="flex gap-4">
                    <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center shrink-0">
                      <span className="font-bold text-sm">{(apt.timeStr || (apt as any).startTime || "10:00").split(" ")[0]}</span>
                    </div>
                    <div>
                      <h4 className="font-bold">{apt.patientName || "Patient"}</h4>
                      <p className="text-sm text-muted-foreground mt-0.5">with {apt.doctor?.name || "Doctor"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
