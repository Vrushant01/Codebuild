...
export default function PatientBookingPage() {
  ...
  const handleDateSelect = async (date: string) => {
    ...
    const exists = availability.some(d => d.date === date)
    if (!exists && doctor) {
      setLoadingAvailability(true)
      const extra = await appointmentService.getAvailability(doctor.id, date, consultationType!, 7)
      setAvailability(prev => {
        const combined = [...prev]
        for (const item of extra) {
          if (!combined.some(c => c.date === item.date)) {
            combined.push(item)
          }
        }
